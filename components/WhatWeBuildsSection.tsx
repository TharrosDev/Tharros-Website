...
function AgentDiagram({ kind }: { kind: "inquiry" | "lead" | "afterhours" }) {
  ...
  return (
    ...
    >
      ...
      {/* per-kind differentiators */}
      <DiagramExtras kind={kind} />

      {/* live signal travelling the flow */}
      <g className="accent">
        <SignalDot
...
function DiagramExtras({ kind }: { kind: "inquiry" | "lead" | "afterhours" }) {
  if (kind === "inquiry") {
    // upward branch off the Agent: it answers in scope, only escalating when it can't.
    return (
      <g className="accent">
  ...
  if (kind === "lead") {
    // a captured-lead check sits on the Inbox node
    return (
      <motion.path
        variants={drawStroke}
        d="M300 60 l4 4 l7 -9"
  ...
  // afterhours: an overnight gap marker between Agent and Summary
  return (
