...
import { useDebounced, useEscape, useFocusTrap, useLockBodyScroll } from "@/lib/hooks";
...
export default function SearchOverlay({ open, onClose }: Props) {
  ...
  const suggestions = getFeatured(4);
  ...
  const remember = (value: string) => {
    ...
    recentStore.set((current) =>
      [trimmed, ...current.filter((entry) => entry !== trimmed)].slice(0, MAX_RECENT),
    );
  };
                ...
                {(searching ? results : suggestions).map((product) => (
                  <li key={product.id}>
                    ...
                    >
                      ...
                      <p className="type-body-sm mt-3 font-medium">{product.name}</p>
