...
- **Field schema** uses an optional `visibleWhen: (state) => boolean` predicate (in `lib/types.ts`) for both fields and whole steps. Hidden fields are treated as satisfied in `stepComplete` and skipped in the merged prompt — see the assistant step's `hoursOfOperation`/`afterHours` for the canonical example.
- **Anti-spam:** `/api/brief` checks a `company_name_alt` honeypot input rendered by `Wizard.tsx` (must stay in the DOM but visually hidden). Rate-limit (`lib/ratelimit.ts`) is wired but disabled until `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` are set.
